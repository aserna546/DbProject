...
class GUI:
    ...
    def makeReservation(self):
        ...
        if not self.allReservation[0]:
            ...
            self.allReservation.append((self.train.get(), self.departDatesv.get(), self.dVar, departsFrom, arrivesAt, self.classVar, pr, self.bags.get(), self.name.get(),0))
        ...
        for result in (self.allReservation):
            ...
            if not cursor.fetchall():
                ...
                lst = list(result)
                lst[8] = bagCost+int(result[6][0])
                result = tuple(lst)
            else:
                self.tcNum = self.tcNum+(bagCost+int(result[6][0]))*0.8
                result[9] = (bagCost+int(result[6][0]))*0.8
    ...
    def selectTree(self, frame):
        tree = Treeview(frame)
        tree.pack(side=TOP)
        tree["columns"] = ("train", "Date", "time", "dept", "arrv", "class", "pr", "bag", "name", "rem")
        tree.heading("train", text="Train (Train Number)")
        ...
        tree.column("train", anchor=CENTER)
        tree.column("Date", anchor=CENTER)
        tree.column("time", anchor=CENTER, width=300)
        tree.column("dept", anchor=CENTER)
        tree.column("arrv", anchor=CENTER)
        tree.column("class", anchor=CENTER)
        tree.column("pr", anchor=CENTER)
        tree.column("bag", anchor=CENTER)
        tree.column("name", anchor=CENTER)
        tree.column("rem", anchor=CENTER)
        return tree
