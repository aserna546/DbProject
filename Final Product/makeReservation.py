...
class GUI:
    ...
    def selectDepart(self):
        ...
        if departsFrom == arrivesAt:
            r = messagebox.showerror("Error!", "Departure and Arrival are the same Station! Choose Different Stations")
        elif self.validatedate(departDate):
            r = messagebox.showerror("Error!", "DATE format is invalid. MUST be YYYY-MM-DD")
        else:
    ...
    def makeReservation(self):
        ...
        if not self.allReservation[0] and self.removeTrue:
        ...
        Button(frame, text="Remove",command=self.removeTic).grid(row=3,column=9,sticky='nsew')
        row = 3
        self.removeLine = IntVar()
        for result1 in (self.allReservation):
            print(result1)
            row += 1
            #for Resd, TrainNum, Class, DepatD, PassName, NumB, DepaF, ArrA in (result1):
            Label(frame, text=result1[0]).grid(row=row, column=0, sticky="nsew")
            Label(frame, text=result1[1]).grid(row=row,column=1,sticky="nsew")
            Label(frame, text=result1[2]).grid(row=row,column=2,sticky="nsew")
            Label(frame, text=result1[3]).grid(row=row,column=3,sticky="nsew")
            Label(frame, text=result1[4]).grid(row=row,column=4,sticky="nsew")
            Label(frame, text=result1[5]).grid(row=row,column=5,sticky="nsew")
            Label(frame, text=result1[6][0]).grid(row=row,column=6,sticky="nsew")
            Label(frame, text=result1[7]).grid(row=row,column=7,sticky="nsew")
            Label(frame, text=result1[8]).grid(row=row,column=8,sticky="nsew")
            Radiobutton(frame, variable=self.removeLine,value=row).grid(row=row, column=9, sticky='nsew')  # ,command=cancelRes)
        ...
        for result in (self.allReservation):
            ...
            if not cursor.fetchall():
                ...
                lst = list(result)
                lst[9] = bagCost+int(result[6][0])
                result = tuple(lst)
                self.allReservation[i]=result
            else:
                self.tcNum = self.tcNum+(bagCost+int(result[6][0]))*0.8
                lst = list(result)
                lst[9] = (bagCost+int(result[6][0]))*0.8
                result = tuple(lst)
                self.allReservation[i] = result
            i=i+1
        ...
        db = self.connect()
        cursor = db.cursor()
        query = "SELECT RIGHT(CardNumber,4) FROM PaymentInfo WHERE Username='%s'" % (self.userNam)
        print(self.userNam)
        ...
        addCardB = Button(frame4, text="Add a Card", command=self.addaCard)
        addCardB.pack(side=RIGHT)
    ...
    def submitRes(self):
        ...
        for result in (self.allReservation):
            sql = "INSERT INTO Reserves (ReservationID,TrainNumber,Class,DepartureDate,PassengerName,NumBags,DepartsFrom,ArrivesAt,TicketPrice)\
                    VALUES\
                    (%i,'%s','%s','%s','%s',%i,'%s','%s',%i);" % (int(resID[0])+1,result[0],result[5],result[1],result[8],int(result[7]),result[3],result[4],result[9])
            cursor.execute(sql)
        ...
        sql = "INSERT INTO Reservation (ReservationID,isCancelled,CardNumber,Username)\
                VALUES\
                (%i, 0,'%s','%s');" % (int(resID[0])+1,wholeCard[0], self.userNam)
        cursor.execute(sql)
