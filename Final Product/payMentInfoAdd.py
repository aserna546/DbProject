...
class GUI:
    ...
    def deleteCard(self):
        ...
        cantdelete = "Select CardNumber,UserName From (Select CardNumber,UserName,ReservationID, Max(DD)" \
                     " as DD,IsCancelled From (Select CardNumber,UserName,Reserves.ReservationID, " \
                     "Max(DepartureDate) as DD,IsCancelled FROM Reserves " \
                     "LEFT JOIN Stop S ON (S.Name=Reserves.ArrivesAt " \
                     "AND S.TrainNumber = Reserves.TrainNumber) LEFT JOIN Stop A " \
                     "ON (A.Name=Reserves.DepartsFrom AND A.TrainNumber = Reserves.TrainNumber) " \
                     "LEFT JOIN TrainRoute T On (T.TrainNumber=Reserves.TrainNumber) LEFT JOIN Reservation R " \
                     "On (R.ReservationID=Reserves.ReservationID) GROUP BY ReservationID) as x GROUP BY " \
                     "CardNumber)as y WHERE CurDate()>Date(DD) OR IsCancelled=1 AND UserName = Daniel_Ericson"
        cursor.execute(query)
